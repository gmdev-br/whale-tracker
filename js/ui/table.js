...
function _renderTableInternal() {
    ...
    function renderCharts(scatterPoints = null, liqPoints = null) {
        renderScatterPlot(scatterPoints);
        renderLiqScatterPlot(liqPoints);
    }
    ...
    if (filterCache.has(cacheKey)) {
        ...
        if (dataWorker) {
            ...
            const filterState = {
                ...
                bubbleScale: (typeof getBubbleScale === 'function' ? getBubbleScale() : 1.0)
            };
    ...
    function finalizeTableRender(rows, showSymbols, skipCharts = false) {
        const currentPrices = getCurrentPrices();
        const fxRates = getFxRates();
        const activeCurrency = getActiveCurrency();
        const activeEntryCurrency = getActiveEntryCurrency();
