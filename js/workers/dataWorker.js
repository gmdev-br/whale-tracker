    ...
    for (let i = 0; i < rows.length; i += CHUNK_SIZE) {
        ...
        const processedChunk = chunk.map(r => {
            ...
            if (!isNaN(coinPrice) && coinPrice > 0) {
                ...
                if (newRow.leverageValue > 0) newRow.marginUsed = newRow.positionValue / newRow.leverageValue;
...
self.onmessage = async function (e) {
    ...
    // 3. Single-Pass Stats & Aggregation
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (!stats.coinStats[r.coin]) stats.coinStats[r.coin] = { totalPositionValue: 0, count: 0, whales: new Set() };
        stats.coinStats[r.coin].totalPositionValue += r.positionValue;
        stats.coinStats[r.coin].count++;
        stats.coinStats[r.coin].whales.add(r.address);
    ...
    for (let i = 0; i < coins.length; i++) {
        const c = coins[i];
        stats.coinStats[c].whaleCount = stats.coinStats[c].whales.size;
        delete stats.coinStats[c].whales;
    }
